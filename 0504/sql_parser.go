...
// 你来实现（把 parseAdd/parseMul 的公共逻辑抽成通用高阶函数）：
//  1. check(len(tokens) == len(ops))
//  2. left = inner()  // inner = 下一层（更高优先级）解析函数，由参数传入
//  3. 循环：对每个 i 试 tryPunctuation(tokens[i]) 或 tryKeyword(tokens[i])——两种都试，
//     因为 token 既有符号(+ - < =)又有关键字(AND OR)；命中就 right = inner()，
//     left = &ExprBinOp{ops[i], left, right}，break 重新扫一轮
//  4. 一轮全不命中就返回 left
