...
// 你来实现（执行 select，对每个输出列求值）：
//  1. 取 schema、按主键 Select 出整行 row（拿不到就返回）
...
// 你来实现（执行 update，「同时赋值」语义——右边都基于旧行求值）：
//  1. 取 schema、按主键 Select 出旧 row
//  2. updates := make([]NamedCell, len(stmt.value))
